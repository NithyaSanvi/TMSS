        ...
        if (schedule_id) {
            getScheduling_Unit_Draft_By_Id(schedule_id).then(scheduleunit =>{
                getTasks_Draft_By_scheduling_Unit_Id(scheduleunit.data.id).then(tasks =>{
                    this.setState({
                        scheduleunit : scheduleunit.data,schedule_unit_task : tasks.data.results
                    });
                })
            })
         }
            ...
            <>
                <div className="p-grid">
                    ...
                    <div className="p-col-1">
                        <Link to={{ pathname: '/sheduling/edit', state: {id: this.state.scheduleunit?this.state.scheduleunit.id:''}}} tooltip="Edit" >
                ...
                */}
                {this.state.schedule_unit_task.length>0 &&
                    <ViewTable 
